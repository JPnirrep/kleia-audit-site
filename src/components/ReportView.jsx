...
const ReportView = ({ data }) => {
    // If data is provided (history), use it. Otherwise use defaults logic.
    // For V2 transition, we keep the existing logic but simulated dynamic behavior.
    ...
    useEffect(() => {
        ...
        // Animate Score
        const finalScore = data?.score || 57; // Use prop data or default
        let currentScore = 0;
    ...
    return (
        <div className="p-5 dashboard-container max-w-[1200px] mx-auto grid grid-cols-12 gap-5 relative">

            {/* BUTTON DOWNLOAD (Hidden for now in V2 or kept? User wanted save history) */}
            ...
            <div className="header-section col-span-12 flex justify-between items-center mb-5 pb-5 border-b border-[var(--border-color)]">
                <div>
                    ...
                    <div className="brand-subtitle text-sm text-[var(--text-muted)]">CIBLE : {data?.url || 'https://leader-speaker.com/mastermind3'}</div>
                    <div className="brand-subtitle text-sm text-[var(--text-muted)]">DATE : <span>{data?.date ? new Date(data.date.seconds * 1000).toLocaleDateString() : currentDate}</span></div>
                </div>
